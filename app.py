...
def upload_resume():
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE results SET resume_score = %s, skills_found = %s WHERE user_email = %s ORDER BY id DESC LIMIT 1",
            (score, ', '.join(found_skills), session['user'])
        )
        conn.commit()
        conn.close()
...
def submit_answers():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO results (user_email, resume_score, skills_found, answers, feedback) VALUES (%s, %s, %s, %s, %s)",
...
def results():
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT resume_score, skills_found, answers, feedback FROM results WHERE user_email = %s ORDER BY id DESC", (email,))
...
def download_report():
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT resume_score, skills_found, answers, feedback FROM results WHERE user_email = %s ORDER BY id DESC LIMIT 1", (email,))
